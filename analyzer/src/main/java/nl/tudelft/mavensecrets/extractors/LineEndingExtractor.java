...
public class LineEndingExtractor implements Extractor {
    ...
    public Object[] extract(Maven mvn, Package pkg, String pkgType, Database db) throws IOException, SQLException {
        ...
        List<JarEntry> entries = jar.stream()
                ...
                .filter(entry -> shouldProcess(pkg.id(), entry.getRealName()))
                .toList();
     ...
     *
     * @param context The package id.
     * @param path Target path.
     * @return If the entry should be processed by this extractor.
     * @throws NullPointerException If <code>path</code> is <code>null</code>.
     */
    private boolean shouldProcess(PackageId context, String path) {
        // Preconditions
        Objects.requireNonNull(context);
        Objects.requireNonNull(path);

        // Not a Maven file, but most likely generated by a Maven plugin
        //if (path.equals("META-INF/MANIFEST.MF")) {
        //    return true;
        //}

        // Maven files
        if (path.equals("META-INF/maven/" + context.group() + '/' + context.artifact() + "/pom.properties")) {
            return true;
        }
